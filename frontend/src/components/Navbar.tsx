    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
      <div className="relative w-full max-w-md bg-card rounded-2xl shadow-2xl overflow-hidden border border-border">
        ...
        <div className="bg-gradient-to-r from-primary to-red-700 px-6 pt-8 pb-6 text-center">
          ...
          <h2 className="text-white text-2xl font-black tracking-tight">Creator Annual Access</h2>
          <p className="text-white/80 text-sm mt-1">Pay once. Create all year.</p>
        </div>
...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-16 xl:px-24 transition-all duration-500 glass shadow-lg shadow-black/5 ${isScrolled ? 'h-14 border-b border-primary/10' : 'h-20'
        } ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>

        {/* Logo / Desktop Nav */}
        <div className="flex items-center gap-8">
          <a href="/" className="text-primary text-2xl md:text-3xl font-black tracking-tighter hover:scale-105 transition-transform duration-200">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          {/* Search — unchanged */}
          <div className={`relative flex items-center transition-all duration-500 ease-in-out ${isSearchFocused ? 'w-56 md:w-80 scale-105' : 'w-10 md:w-48'}`}>
            <MdSearch className={`absolute left-3 text-xl transition-colors duration-300 pointer-events-none z-10 ${isSearchFocused ? 'text-primary' : 'text-muted-foreground'}`} />
            <input
              type="text"
              placeholder={isSearchFocused ? 'Search titles, actors, genres...' : ''}
              onFocus={() => setIsSearchFocused(true)}
              onBlur={() => setIsSearchFocused(false)}
              className={`w-full bg-secondary/40 border border-white/5 rounded-full py-2 pl-10 pr-4 text-sm transition-all duration-500 outline-none ${isSearchFocused
        ...
        <div className="fixed inset-0 z-[60] flex md:hidden">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-72 bg-card border-l border-border flex flex-col shadow-2xl animate-slide-in">
            ...
            <div className="px-4 pb-6 border-t border-border pt-4 flex flex-col gap-3">
              <button
                onClick={() => { setMobileMenuOpen(false); setShowWaitlist(true); }}
              ...
              <button
                onClick={() => { setMobileMenuOpen(false); navigate('/creator-login'); }}
